...
다형적 참조
- 부모타입 자신은 물론이고 자신을 기준으로 모든 자식 타입을 참조할 수 있다.

업캐스팅은 객체가 생성될때 부모클래스 모두 생성되기 때문에 문제가 되지않음.
다운캐스팅은 인스턴스에 존재하지 않는 하위타입으로 캐스팅하는 문제가 발생할 수 있으므로 문제를 인지하고 사용해야함.

자식 클래스에서 오버라이딩 된 메서드는 항상 우선권을 가진다. 오버라이딩이란 부모 클래스에서 정의한 메서드를 자식 클래스에서 재정의 하는것이다. 가장 하위의 자식 클래스 인스턴스의 오버라이딩 된 메서드가 우선권을 가진다.
업캐스팅하여 부모클래스의 메서드를 사용한다 하더라도 오버라이딩 된 경우 부모클래스 인스턴스의 메서드를 사용하는게 아닌 자식 클래스에서 오버라이딩된 메서드가 실행된다.
...
인터페이스
- 인터페이스의 메서드는 모두 public, abstract이다. -> 따라서 생략 가능
- 인터페이스는 다중 구현(다중 상속)을 지원한다.
- 인터페이스의 멤버 변수는 public, static, final이 모두 포함되어 있다고 간주한다.(상수) -> 따라서 생략 가능

인터페이스는 다중 구현을 허용한다.
다중 상속의 경우 여러 부모 클래스의 메서드 중에 어떤걸 사용해야할지 결정해야 하는 다이아몬드 문제가 발생한다.
인터페이스는 여러 부코클래스가 있다고 하더라도 자식클래스에서 무조건 오버라이딩된 메서드를 사용하기 때문에 상관이 없어서 다중구현을 허용한다.